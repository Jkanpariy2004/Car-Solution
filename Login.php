...
if (isset($_REQUEST['btnlogin'])) {
    ...
    if (empty($errors)) {
        $sql = "select * from admin_login where email='$email' AND BINARY password='$password'";
        $result = mysqli_query($db, $sql);

        if (mysqli_num_rows($result)) {
            $_SESSION['email'] = $email;
            header("location:user.php");
            // $errors['success'] = "Login successful. Please Wait 10 seconds.";
        } else {
    ...
    <div class="login">
        <form method="POST" class="loginbg p-4 wow fadeIn" data-wow-delay="0.1s" style="max-width: 50%;">
            ...
            <div class="form-floating mb-3">
                <input type="email" class="form-control rounded-3 <?php if (isset($errors['email'])) echo 'is-invalid'; ?>" autocomplete="off" id="floatingInput" name="email" placeholder="" value="<?php echo htmlspecialchars($email); ?>">
            ...
            <div class="m-auto">
                <input type="submit" name="btnlogin" class="btn w-100  btn-primary fw-bold fs-5 btn mt-3 loginbtn rounded-3" value="Login">
            </div>
            <div class="text-center">
                <a type="submit" href="Login.php" class="btn btn-danger btn w-25 mt-3 loginbtn rounded-3">Admin Login</a>
