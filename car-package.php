...
$view = "Select * from car_package";
$GetData = mysqli_query($db, $view);

if (isset($_REQUEST['delete'])) {
    $id = $_REQUEST['delete'];

    $sql = "delete from car_package where id='$id' ";
    mysqli_query($db, $sql);
    header("location:car-package.php");
}

if (isset($_REQUEST['success']) || isset($_REQUEST['pending'])) {
    if (isset($_REQUEST['success'])) {
        $id = $_REQUEST['success'];
        $updateQuery = "update car_package set status='success' where id='$id'";
    } elseif (isset($_REQUEST['pending'])) {
        $id = $_REQUEST['pending'];
        $updateQuery = "update car_package set status='pending' where id='$id'";
    }
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr class="table-light border-bottom border-dark">
                                            ...
                                            <td><a href="car-package.php?delete=<?php echo $row['id'] ?>" onclick=" return confirm('Are You Sure Record Delete !!!') " class="fs-2 text-dark active" role="button" aria-pressed="true"><i class='bx bx-trash'></i></a></td>
                                        </tr>
                                    <?php
                                    }
                                    ?>
