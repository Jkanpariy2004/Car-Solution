...
$view = "Select * from add_car_slider";
...
if (isset($_REQUEST['btnsubmit'])) {
    ...
    $sql = " insert into add_car_slider(image,name) values ('$image','$name') ";
    mysqli_query($db, $sql);

    echo "<script>
        alert('Data insert Successfully..');
    </script>";
    header("location:add-car-slider.php");
}

if (isset($_REQUEST['delete'])) {
    $id = $_REQUEST['delete'];
    $editimg = "select *from add_car_slider where id='$id'";
    $img1 = mysqli_query($db, $editimg);
    $return = mysqli_fetch_array($img1);
    unlink('image/' . $return['image']);

    $sql = "delete from add_car_slider where id='$id' ";
    mysqli_query($db, $sql);
    header("location:add-car-slider.php");
}
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr class="table-light border-bottom border-dark">
                                            ...
                                            <td><a href="add-car-slider-edit.php?edit=<?php echo $row['id'] ?>" class="fs-2 text-dark active" role="button" aria-pressed="true"><i class='bx bx-edit'></i></a></td>
                                            <td>
                                                <a href="add-car-slider.php?delete=<?php echo $row['id'] ?>" onclick="return confirm('Are You Sure Record Delete !!!')" class="fs-2 text-dark active" role="button" aria-pressed="true">
